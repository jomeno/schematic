...
public class ContentProviderWriter {
  ...
  private MethodSpec getBulkInsertSpec() {
    MethodSpec.Builder spec = MethodSpec.methodBuilder("bulkInsert")
        ...
        .addParameter(ArrayTypeName.of(Clazz.CONTENT_VALUES), "values")
        .addStatement("$T db = database.getWritableDatabase()", Clazz.SQLITE_DATABASE)
  ...
  private MethodSpec getQuerySpec() {
    ...
    spec.addStatement("$T db = database.getReadableDatabase()", Clazz.SQLITE_DATABASE);
    ...
    for (UriContract uri : uris) {
      if (uri.allowQuery) {
        ...
        spec.addStatement("final String groupBy = $S", String.class, uri.groupBy)
            .addStatement("final String having = $S", String.class, uri.having)
            .addStatement("final String limit = $S", String.class, uri.limit);

        // TODO: The whereBuilder part is kind of gross
        spec.addStatement(
            "$T cursor = builder.table($S)$L.query(db, projection, groupBy, having, sortOrder, limit)",
  ...
  private MethodSpec getUpdateSpec() {
    ...
    for (UriContract uri : uris) {
      if (uri.allowUpdate) {
        ...
        spec.addStatement("final $T count = builder.table($S).update(db, values)", int.class,
